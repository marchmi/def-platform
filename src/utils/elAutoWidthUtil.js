 ...
 * @param {String} font The css font descriptor that text is to be rendered with (e.g. "normal 14px PingFang SC").
 *
 */
export const getTextWidth = (text, font = 'normal 14px PingFang SC') => {
...
export const setElMinWidth = ( fieldsInfo, selector = '.el-table__header', padding = 36 ) => {
 ...
 export const resetDescPanlLabelWidth = () => {
   ...
   let maxWidth = 0
   els.forEach(el => {
     const textWidth = getTextWidth(el.innerText || '', `normal ${fontSize}px PingFang SC`)
 ...
 export const resetOperatiosColumnWidth = (val, moreThanNum = 0, moreButtonText) => {
   ...
   labels.forEach(label => {
     totalWidth = totalWidth + getTextWidth(label, 'normal 14px PingFang SC') + 24 // 每个按钮的左右边距24
   })
   // let textWidth = getTextWidth(val.label, 'normal 14px PingFang SC')
   return totalWidth
 }
 ...
 export const resetTableHeaderTitleWidth = (columns) => {
   columns.forEach(col => {
     const result = getTextWidth(col.label ? col.label : '', 'bold 14px PingFang SC') + 40 // 40的边距
 ...
 export const resetfilterLabelWidth = (columns) => {
   ...
   columns.forEach(col => {
     // 遍历筛选条件的label
     let textWidth = getTextWidth(col.label ? col.label : '', 'normal 14px PingFang SC')
     ...
     if (col.type === 'select-input') {
       for (const val in col.options) {
         textWidth = getTextWidth(col.options[val], 'normal 14px PingFang SC')
         if (textWidth > maxWidth) {
           maxWidth = textWidth
         }
       }
